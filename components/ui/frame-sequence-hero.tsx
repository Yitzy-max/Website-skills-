...
/**
 * FrameSequenceHero — the MacBook Neo scroll hero, unchanged.
 *
 * The scrub engine below is the original component verbatim: same progress
 * math, same 0.28 lerp, same 0.08 snap, same eager-preload strategy, same
 * step windows. Two additions, both additive:
 *
 *   1. `aside` — an optional node rendered alongside a chosen step (used for
 *      the customer review that rides with the first dish).
 *   2. `asideStep` — which step index that node belongs to (default 0).
 *
 * Styles live in tapas/css/hero.css — paste that file into your globals.css.
 * Class names are identical between this and the vanilla build.
 */
...
export function FrameSequenceHero({
  ...
  return (
    <div className={cx("fsh-root", className)}>
      ...
      {/* The spacer owns the scroll distance and the stage sticks inside it,
          so the hero releases the page at progress 1 instead of staying
          fixed over the sections below. */}
      <div ref={spacerRef} className="fsh-spacer" style={{ height: scrollHeight }}>
        <div className="fsh-stage">
          ...
          {aside && (
            <aside
              className={cx("fsh-review", activeIdx === asideStep && "fsh-review-show")}
            >
              {aside}
            </aside>
          )}
